import { useState } from "react";
import { Sidebar } from "./sidebar";
import { Header } from "./header";
import { TitleBar } from "./title-bar";
import { motion } from "framer-motion";

interface MainLayoutProps {
  children: React.ReactNode;
}

export function MainLayout({ children }: MainLayoutProps) {
  const [isPinned, setIsPinned] = useState(false);
  const [isHoverOpen, setIsHoverOpen] = useState(false);

  const isVisible = isPinned || isHoverOpen;

  const handleMouseEnter = () => !isPinned && setIsHoverOpen(true);
  const handleMouseLeave = () => !isPinned && setIsHoverOpen(false);

  return (
    <div className="flex h-screen w-full bg-background overflow-hidden font-lexend text-foreground transition-colors duration-300">
      <TitleBar />

      {!isPinned && (
        <div
          className="fixed left-0 top-0 mt-9 z-50 h-[calc(100vh-2.25rem)] w-40"
          onMouseEnter={handleMouseEnter}
        />
      )}

      <div className="pt-9 z-40 relative">
        <Sidebar
          isVisible={isVisible}
          isPinned={isPinned}
          onTogglePin={() => {
            setIsPinned(!isPinned);
            if (isPinned) setIsHoverOpen(true);
          }}
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        />
      </div>

      <div className="flex flex-1 flex-col overflow-hidden transition-all duration-300 ease-in-out pt-9">
        <Header />

        <motion.main
          className="flex-1 overflow-y-auto p-6"
          animate={{ paddingLeft: isPinned ? "288px" : "0px" }}
          transition={{ duration: 0.3, ease: "easeInOut" }}
        >
          {children}
        </motion.main>
      </div>
    </div>
  );
}
